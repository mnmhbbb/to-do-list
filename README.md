...
#### 느낀 점   
원하는 기능을 넣으려고 할수록 공부해야 할 부분이 연쇄적으로 발생했다. 부족함을 많이 느꼈다. 어떻게든 기능을 동작시키기는 했지만, 스스로 생각하기에 코드가 비효율적이라고 느껴졌다. 조금 더 간결하고 깔끔하게 입력해서, 며칠이 지나고 다시 봐도 이해가 쉽고 가독성이 좋은 코드를 짜고 싶다.   
또한 함수를 여러 개 만들었지만, 각 기능이 동작하는 순서를 헷갈려서 오류를 발생시켰고 결국 많은 시간이 지체되었다. 이 또한, 내가 내 코드를 제대로 읽지 못해서 발생한 문제라는 생각이 든다. 다른 예제들을 보면서 각 상황별로 정리하는 방법을 습득해야겠다.   
